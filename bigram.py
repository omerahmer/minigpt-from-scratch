...
def get_batch(split):
    ...
    y = torch.stack(
        [torch.from_numpy(data[i + 1 : i + block_size + 1]) for i in ix]
    ).long()
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # x holds token identities and positions where tokens occur
        # logits are the list of scores for each next token for a given token
        x = self.blocks(x)
        x = self.ln_f(x)
        ...
        if targets is None:
            ...
            # reshaping b/c pytorch expects B, C, T
            B, T, C = logits.shape
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            # focus only on last time step (since we only want to generate from last token)
            logits = logits[:, -1, :]  # becomes (B, C)
            # apply softmax to get probabilities from scores
